...
if (productCount.count === 0) {
  ...
  const initialProducts = db.prepare(
    `INSERT INTO products (name, image, description, price, tags) VALUES (?, ?, ?, ?, ?)`
  );
  const products = [
        {
  ...
  products.forEach((product) => initialProducts.run(product.name, product.image, product.description, product.price, product.tags));
}
